...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 计算应充值额度：
		// - Stripe/C2Coin 订单：Money 代表经分组倍率换算后的美元数量，直接 * QuotaPerUnit
		// - 其他订单（如易支付）：Amount 为美元数量，* QuotaPerUnit
		if topUp.PaymentProvider == PaymentProviderStripe || topUp.PaymentProvider == PaymentProviderC2Coin {
...
// GetPendingUsdtTopUps returns all pending USDT orders ordered by creation time (oldest first).
func GetPendingUsdtTopUps() ([]*TopUp, error) {
	var topUps []*TopUp
	err := DB.Where("status = ? AND payment_provider = ?", common.TopUpStatusPending, PaymentProviderUSDT).
