...
const paymentSchema = z.object({
  ...
  UsdtUnitPrice: z.coerce.number().min(0),
  UsdtMinTopUp: z.coerce.number().min(1),
  UsdtInstructions: z.string(),
...
export function PaymentSettingsSection({
  ...
  const onSubmit = async (values: PaymentFormValues) => {
    const sanitized = {
      ...
      UsdtNetwork: values.UsdtNetwork.trim() || 'TRC20',
      UsdtAddress: values.UsdtAddress.trim(),
      UsdtUnitPrice: values.UsdtUnitPrice,
      UsdtMinTopUp: values.UsdtMinTopUp,
      UsdtInstructions: values.UsdtInstructions.trim(),
    ...
    const initial = {
      ...
      UsdtNetwork: initialRef.current.UsdtNetwork.trim() || 'TRC20',
      UsdtAddress: initialRef.current.UsdtAddress.trim(),
      UsdtUnitPrice: initialRef.current.UsdtUnitPrice,
      UsdtMinTopUp: initialRef.current.UsdtMinTopUp,
      UsdtInstructions: initialRef.current.UsdtInstructions.trim(),
    ...
    if (sanitized.UsdtEnabled !== initial.UsdtEnabled) {
      updates.push({ key: 'UsdtEnabled', value: sanitized.UsdtEnabled })
    }

    if (sanitized.UsdtNetwork !== initial.UsdtNetwork) {
      updates.push({ key: 'UsdtNetwork', value: sanitized.UsdtNetwork })
    }

    if (sanitized.UsdtAddress !== initial.UsdtAddress) {
      updates.push({ key: 'UsdtAddress', value: sanitized.UsdtAddress })
    }

    if (sanitized.UsdtUnitPrice !== initial.UsdtUnitPrice) {
      updates.push({ key: 'UsdtUnitPrice', value: sanitized.UsdtUnitPrice })
    }

    if (sanitized.UsdtMinTopUp !== initial.UsdtMinTopUp) {
      updates.push({ key: 'UsdtMinTopUp', value: sanitized.UsdtMinTopUp })
    }

    if (sanitized.UsdtInstructions !== initial.UsdtInstructions) {
      updates.push({
        key: 'UsdtInstructions',
        value: sanitized.UsdtInstructions,
      })
    }
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {t(
                          'Configured as PayMethods JSON. The type value decides which payment flow is used: stripe for Stripe, waffo_pancake for Waffo Pancake, usdt for manual USDT, and other values are sent to Epay as the type parameter.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
            ...
            <TabsContent value='usdt' className={paymentTabContentClassName}>
              <div className='space-y-4'>
                <div>
                  ...
                  <p className='text-muted-foreground text-sm'>
                    {t('Manual USDT top-up with pending orders and admin confirmation')}
                  </p>
                </div>

                <Alert>
                  <ShieldAlert className='h-4 w-4' />
                  <AlertTitle>{t('USDT safety reminder')}</AlertTitle>
                  <AlertDescription>
                    {t(
                      'This gateway creates pending orders only. Verify the network, amount, and transaction hash before completing an order in the admin top-up list.'
                    )}
                  </AlertDescription>
                </Alert>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className='flex items-center justify-between gap-4'>
                        <SettingsSwitchContent
                        >
                          <FormLabel>{t('Enable USDT top-up')}</FormLabel>
                          <FormDescription>
                            {t(
                              'Show USDT as a payment method after a receiving address is configured.'
                            )}
                          </FormDescription>
                        </SettingsSwitchContent>
                        <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('Common value: TRC20. Keep this exact for customer clarity.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('Customers will transfer USDT to this address.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className='grid gap-6 md:grid-cols-2'>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('USDT price per USD balance')}</FormLabel>
                        <FormControl>
                          <Input
                            type='number'
                            step='0.01'
                        ...
                        <FormDescription>
                          {t('Use 1 for 1 USDT = 1 USD balance before group ratio and discounts.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('Smallest amount users can recharge with USDT.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  control={form.control}
                  name='UsdtInstructions'
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Customer instructions')}</FormLabel>
                      <FormControl>
                        <Textarea
                          rows={4}
                          {...field}
                      ...
                      <FormDescription>
                        {t('Shown in the USDT payment dialog after an order is created.')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
