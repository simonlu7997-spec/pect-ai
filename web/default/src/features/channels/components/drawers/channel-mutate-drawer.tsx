...
const DGX_SPARK_CHANNEL_PRESET = {
  name: 'DGX Spark GB10',
  type: 1,
  base_url: 'http://spark.local:8000',
  models: DGX_SPARK_MODELS.join(','),
  group: ['default'],
  test_model: 'qwen3.6:35b',
  tag: 'spark',
  remark:
    'Private DGX Spark upstream. Use a LAN/VPN URL; do not expose this server publicly.',
}
...
export function ChannelMutateDrawer({
  ...
  const applyDgxSparkPreset = useCallback(() => {
    form.setValue('name', DGX_SPARK_CHANNEL_PRESET.name, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('type', DGX_SPARK_CHANNEL_PRESET.type, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('base_url', DGX_SPARK_CHANNEL_PRESET.base_url, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('models', DGX_SPARK_CHANNEL_PRESET.models, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('group', DGX_SPARK_CHANNEL_PRESET.group, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('test_model', DGX_SPARK_CHANNEL_PRESET.test_model, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('tag', DGX_SPARK_CHANNEL_PRESET.tag, {
      shouldDirty: true,
      shouldValidate: true,
    })
    form.setValue('remark', DGX_SPARK_CHANNEL_PRESET.remark, {
      shouldDirty: true,
      shouldValidate: true,
    })
    ...
    toast.success(t('DGX Spark preset applied'))
  }, [form, t])
          ...
          {!isEditing && (
            <Alert>
              <AlertDescription className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                <span>
                  {t(
                    'Use the DGX Spark preset to add your private OpenAI-compatible Spark API as an upstream channel.'
                  )}
                </span>
                <Button
                  type='button'
                  ...
                  size='sm'
                  className='shrink-0'
                  onClick={applyDgxSparkPreset}
                >
                  <Sparkles className='size-4' />
                  <span>{t('DGX Spark Preset')}</span>
                </Button>
              </AlertDescription>
            </Alert>
          )}
